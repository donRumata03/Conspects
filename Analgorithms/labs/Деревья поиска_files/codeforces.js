...
/* Code duplicated: see CodeforcesIframeInit.js */
function appendParameterToUrl(url, parameterName, parameterValue) {
    if (!parameterName || parameterName.length === 0) {
        return url;
    }

    var parameter = !parameterValue || parameterValue.length === 0
        ? encodeURIComponent(parameterName)
        : encodeURIComponent(parameterName) + "=" + encodeURIComponent(parameterValue);

    var questionSignPos = url.indexOf("?");
    ...
    if (questionSignPos === -1 && sharpPos === -1) {
        ...
        var resultUrl = url.substring(0, questionSignPos + 1) + parameter;
...
Codeforces = new function () {
    ...
    this.showAnnouncements = function (json, locale) {
        var div = document.getElementsByClassName("notificationTextCleaner")[0];
                ...
                if (val.length > 0) {
                    ...
                    for (var i in classes) {
                        if (classes[i].match(/for__.*/)) {
                            ...
                            $(".table-form select[name=" + name + "]").focus();
                            $(".table-form textarea[name=" + name + "]").focus();
    ...
    this.reloadAndShowMessage = function (message) {
        this.putToStorage("delayedMessage", message);
        window.location.reload();
    };

    this.reloadAndShowMessageOrShowError = function (json, message) {
        if (json && json["success"] === "true") {
            if (message) {
                Codeforces.reloadAndShowMessage(message);
            } else {
                Codeforces.reload();
            }
        } else {
            for (let key in json) {
                if (json.hasOwnProperty(key) && key.substring(0, "error__".length) === "error__" && json[key]) {
                    Codeforces.showMessage(json[key]);
                    break;
                }
            }
        }
    ...
    this.showDelayedMessage = function () {
        var message = this.extractFromStorage("delayedMessage");
    ...
    this.redirectAndShowMessage = function (link, message) {
        this.putToStorage("delayedMessage", message);
        this.redirect(link);
    };
    ...
    this.setAjaxFormField = function (selector, key, value) {
        const that = this;
        const $selector = $(selector);
        $selector.find("input[name='" + key + "']"
            + ", select[name='" + key + "']"
    ...
    this.setAjaxFormErrors = function (selector, data) {
        ...
        for (var i in data) {
            if (data.hasOwnProperty(i) && i.indexOf("error__") === 0) {
    ...
    this.setAjaxFormComplete = function (selector, data) {
        this.clearAjaxFormErrors(selector);
        for (const i in data) {
            if (data.hasOwnProperty(i)) {
                if (i.indexOf("error__") === 0) {
                    ...
                    this.setAjaxFormField(selector, i, data[i]);
                }
            }
        }
    };

    this.getAjaxFormFields = function (selector) {
        var result = {};
    ...
    this.extractFromStorage = function (key) {
...
//            window.localStorage[key] = defaultValue;
    ...
    this.putToStorageTtl = function (key, value, seconds) {
        if (window.localStorage) {
            window.localStorage[key] = value;

            let keys = window.localStorage["putToStorageTtl_keys"];
            if (!keys) {
                keys = {};
            } else {
            ...
            keys[key] = now + seconds * 1000;
            window.localStorage["putToStorageTtl_keys"] = JSON.stringify(keys);
        }
    };
    ...
    this.filterClipboardText = function (text, lineCount) { // Because of unexpected bug in prettyprint or Clipboard lib.
    ...
    this.mathJaxListeners = [];
    this.mathJax = false;

    this.addMathJaxListener = function (listener) {
        if (this.mathJax) {
            listener();
        } else {
            this.mathJaxListeners.push(listener);
        }
    }

    this.runMathJaxListeners = function () {
        this.mathJax = true;
        this.mathJaxListeners.forEach(function (listener) {
            listener();
        })
    }
};
