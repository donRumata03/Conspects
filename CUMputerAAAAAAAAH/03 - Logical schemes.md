...
Используются полевые транзисторы n-типа.

Построим элемент НЕ (транзистор, резистор).

Логика может быть как позитивная (потенциал единицы больше, чем у единицы, используется в NMOS), так и негативная - наоборот, .



Не накапливается ли ошибка? Скорее всего, нет, потому что, чтобы открыть транзистор, достаточно не такого большого напряжения.

В чём проблема NMOS логики? Потребляет много энергии. Например, в данном случае при единичном входе там течёт немалый ток, то есть потребляется энергия.

Эту проблему решает CMOS-логика

___

### [CMOS](https://en.wikipedia.org/wiki/CMOS)-логика

Используются 2 типа тразисторов (с $p$ и $n$ каналами)

Элемент НЕ состоит из двух транзисторов.

> Основная энергия потребляется при переключении состояний. Именно поэтому потребление энергии примерно линейно растёт при увеличении тактовой частоты.

В качестве базы используются элементы "ИЛИ-НЕ" и "И-НЕ" (они требуют меньшего количества элементов).

 Более того, очевидно, как сделать элемент "3-ИЛИ-НЕ" из "2-ИЛИ-НЕ", и это выгоднее, чем подключать их в последовательность.

### Дребезг контактов

Старые контакты имеют ещё более длительный переходный процесс.

Можно с ними бороться программно, но это плохая идея.

Обычно это делается на уровне устройства (например, мышки).

Задача решается триггером. Поставим контакт между двумя пластинами. Контакт болтается не так сильно, чтобы при болтании касалось одновременно двух. Будет только по очереди касаться одного или не касаться вообще. Либо то же для другого.

Тогда просто подключим триггер, на его входе будут сигналы (01 и 00) или (10 и 00), поэтому он будет выдавать напряжение питания.

Чтобы оно отключалось когда нужно, достаточно добавить транзистор на входе 

![image-20211016140731587](images/debouncer.png)
...
Если классическая 6T-ячейка памяти (`6t memory cell`).

Если коннекторы `col` и `row`



Существует также динамическая память. Но её чтение может повлиять на её состояние.

Поэтому полезно иногда регенерировать состояние.



Например, за это может отвечать программист (так было давно): нельзя долго не использовать какую-то часть памяти.

Гениальный вариант: это делает пользователь (на компьютере есть кнопка, нажатие которой производит регенерацию, и, когда пришло время отойти за кофе).

Более нормальный вариант - модуль памяти всё это инкапсулирует в себе.