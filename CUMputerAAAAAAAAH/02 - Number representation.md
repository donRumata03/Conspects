...
### Заметки по тесту о плавающих числах

Считать квадратный корень на калькуляторе было нельзя, а написать такую программу, почему-то, можно:
...
Возводить в квадрат нам, почему-то, можно, а брать корень - нет...



Далее - задание с округлением.

Сначала записываем в бинарном виде число больше на единицу, но с той же разрядностью, потом - на один меньше, но тоже с той же. Потом смотрим числа с большей разрядностью, которые всё ещё округляются к данному, а не больше или меньше. Переводим их в десятичную систему.  Записываем эти числа в диапазон. Затем записываем в ответ число, котороые 



> Лайфхак: если, например, записывать константу в тип float, то плохая идея записывать больше десятичных знаков, чем то, что достаточно для однозначного определения того, что в итоге запишется в эту константу. Для этого как раз можно применить вышеописанный алгоритм.
>
> 
>
> Говорят, в современных языках не выводят слишком много десятичных чисел, чтобы не было информационного шума. Например, джава или 
>
> 
>
> А ещё - можно использовать логарифм, чтобы примерно понять, сколько знаков в другой СС требуется.
>
> Например, знаки в 10-ной примерно в $\frac{10}{3}$ раза более "весомые", чем в двоичной. 
> $$
> digits_{2} = digits_2 \cdot \log_2 10 \approx digits_2 \cdot 3.3219280948873626
> $$

____

Следующая тема - логические схемы.
